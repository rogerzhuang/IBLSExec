...
class MarketDataApp(EClient, wrapper.EWrapper):
    ...
    def tickPrice(
        ...
        pass
        # print(f"{reqId} {TickTypeEnum.toStr(tickType)} price: {price} {attrib}")

    @iswrapper
    def tickSize(self, reqId: TickerId, tickType: TickType, size: int):
        if TickTypeEnum.toStr(tickType) == "SHORTABLE_SHARES":
            print(f"{reqId} {TickTypeEnum.toStr(tickType)} size: {size}")

    @iswrapper
    def tickString(self, reqId: TickerId, tickType: TickType, value: str):
        pass
        # print(f"{reqId} {TickTypeEnum.toStr(tickType)} string: {value}")

    @iswrapper
    def tickGeneric(self, reqId: TickerId, tickType: TickType, value: float):
        pass
        # print(f"{reqId} {TickTypeEnum.toStr(tickType)} generic: {value}")
    ...
    def start(self):
        ...
        for contract in self.contracts:
            ...
            self.reqMktData(rid, contract, "236", self.args.snapshot, False, [])
            self.pending_ends.add(rid)
            self.request_contracts[rid] = contract

    @iswrapper
    def error(self, req_id: TickerId, error_code: int, error: str, advancedOrderRejectJson=None):
